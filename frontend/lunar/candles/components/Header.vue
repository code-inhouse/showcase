<template>
  <header id="header" class="header">
    <div class="header__wrapper">
	  ...
	  <span>Lunar Labs Alpha | </span>
        <span class="pair" v-if="pair">{{ pair }}</span>
        <span class="icon-quote"></span>
        <span v-html="price || 'Lunar Live'"></span>
		<span class="avglabel">Index Price</span>
      </div>
      <dropdown
        :options="timeframes"
        :selected="timeframe"
...
header#header {
  ...
  position: relative;
  z-index: 2;
  border-bottom: 1px lighten($dark, 50%);
  ...
  button {
    ...
    &.active {
      ...
      .icon-volume-muted {
        &:before {
          content: unicode($candles-icon-volume);
        }
      }
