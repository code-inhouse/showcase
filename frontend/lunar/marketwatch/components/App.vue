...
export default {
  ...
  methods: {
    ...
    updateOptions( options ) {
      ...
      this.$marketwatchNotify.setOptions( {
    ...
    setupRoutes() {
      // page routes
      this.$marketRouter.on( '/', () => { this.showPage( 'TokenList', 'Price List' ) } );
      this.$marketRouter.on( '/news', () => { this.showPage( 'NewsPage', 'Twitter News' ) } );
      //this.$marketRouter.on( '/trade', () => { this.showPage( 'TradePage', 'Trade Bot' ) } );
      // modal routes
      this.$marketRouter.on( '/history', () => { this.showModal( 'HistoryPage', 'Recent Alert History' ) } );
      this.$marketRouter.on( '/alarms', () => { this.showModal( 'AlarmsList', 'Saved Price Alarms' ) } );
      this.$marketRouter.on( '/about', () => { this.showModal( 'AboutPage', 'About This App' ) } );
      this.$marketRouter.on( '/options', () => { this.showModal( 'OptionsPage', 'Options & Settings' ) } );
      // symbol modal route
      this.$marketRouter.on( '/symbol/([A-Z]+)', symbol => {
        let d = this.priceData.filter( p => p.symbol === symbol ).shift();
        if ( d ) return this.showModal( 'TokenPage', d.pair +' Info ', d );
        this.$marketRouter.setRoute( '/' );
      });
    },
    ...
    tickerNotify( title, message ) {
      ...
      let d = new Date();
      this.$marketwatchNotify.add( title, message +' \nNow: '+ d.toLocaleString() );
    },
    ...
    checkPairAlarms( pair ) {
      this.$alarms.check( pair.symbol, pair.close, ( title, info, a ) => {
        ...
        this.$marketwatchNotify.add( title, info, icon, e => { this.setRoute( '/history' ) } );
    ...
    fetchSentimentWords() {
      Array( 'words' ).forEach( file => {
        this.$ajax.get( '/static/assets/marketwatch/afinn/'+ file +'.json', {
          type: 'json',
          proxy: false,
  ...
  mounted() {
    this.$marketRouter.trigger( window.location.hash || '/' );
