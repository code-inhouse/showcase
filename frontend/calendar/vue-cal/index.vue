<template lang="pug">
  .vuecal(column :class="cssClasses" :lang="locale")
    .vuecal__body(v-if="!hideBody" grow)
      span.arrow-down(v-if="!scrollBottom" v-scroll-to="{el: '#bottom-hours', container: '#top-container'}") ⌃
      transition(:name="`slide-fade--${transitionDirection}`" :appear="transitions")
        div(style="min-width: 100%" :key="transitions ? view.id : false" column)
          ...
          .vuecal__bg(column)
            .calendar__viewport#top-container(row grow @scroll="onScroll")
              span#top-hours
              span.arrow-up(v-if="!scrollTop" v-scroll-to="{el: '#top-hours', container: '#top-container'}") ⌃
              span.arrow-right(v-if="!scrollRight" v-scroll-to="{el: '#right-hours', container: '#top-container', x: true, y: false}") ⌃
              vuecal-header(
                :vuecal-props="$props"
              ...
              .vuecal__flex.vuecal__cells(:class="`${view.id}-view`" grow :wrap="!hasSplits || view.id !== 'week'" :column="hasSplits")
                ...
                .vuecal__flex.calendar__content(grow :wrap="!hasSplits || view.id !== 'week'")
                  .vuecal__grid-lines(v-if="hasTimeColumn")
                    .vuecal__grid-line(v-for="(cell, i) in timeCells" :key="i" :style="`height: ${timeCellHeight}px`")
                      span.line
                  vuecal-cell(
                    :class="cell.class"
                    v-for="(cell, i) in viewCells"
                    ...
                    div(slot="event-renderer" slot-scope="{ event, view }" :view="view" :event="event" class="vuecal__event-title--wrapper" :class="{small: event.height < 40}")
                      slot(name="event-renderer" :view="view" :event="event")
                        .vuecal__event-title.vuecal__event-title--edit(
                          v-if="editableEvents && event.title"
                          contenteditable
...
export default {
  ...
  methods: {
    onScroll ({ target: { scrollTop, scrollLeft, clientHeight, clientWidth, scrollHeight, scrollWidth }}) {
    ...
    weekDays () {
      ...
      if (this.isPlan) {
        ...
        if (dayIndex == 0) {
          ...
          const newArr = weekDays.splice((dayIndex - 1) - 7, 7 - (dayIndex - 1))
          weekDays = newArr.concat(weekDays)
        }

        const diffTime = Math.abs(new Date(this.endPlanDate).getTime() - new Date(this.startPlanDate).getTime())
        const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
        weekDays.splice(diffDays - 1 - 7, 7 - diffDays - 1)

        if (diffDays > 7) {
          let newDaysArr = []
          for (let [i, v] of Array(diffDays + 1).entries()) {
            if (i >= 7) {
              newDaysArr.push(weekDays[i % 7])
            } else {
              newDaysArr.push(weekDays[i])
            }
          }
          weekDays = newDaysArr
        }
    ...
    viewCells () {
      ...
      switch (this.view.id) {
        ...
        case 'week':
          ...
          let d = new Date(this.startPlanDate)
          d.setTime( d.getTime() + d.getTimezoneOffset()*60*1000 );
          let firstDayOfWeek = this.isPlan ? d : this.view.startDate
  ...
  filters: {
    truncate(value, id) {
        ...
        // console.log(countLines(document.getElementById(`event-${id}`)))
